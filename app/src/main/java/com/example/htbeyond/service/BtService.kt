...
import com.example.htbeyond.Constants.CLIENT_CHARACTERISTIC_CONFIG
...
class BtService(private val myRepository: BluetoothRepository) : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        when (intent?.action) {
            ...
            Constants.Actions.STOP_FOREGROUND -> {
                stopForegroundService()
            }
            Constants.Actions.DISCONNECT_DEVICE->{
                disconnectGattServer("Disconnected")
            }
            Constants.Actions.START_NOTIFICATION->{
                startNotification()
            }
            Constants.Actions.STOP_NOTIFICATION->{
                stopNotification()
            }
            Constants.Actions.WRITE_DATA->{
    ...
    private fun broadcastUpdate(action: String, msg: String) {
        ...
        intent.putExtra(Constants.Actions.MSG_DATA, msg)
        sendBroadcast(intent)
    }
    private fun broadcastUpdate(action: String, readBytes: ByteArray) {
        val intent = Intent(action)
        intent.putExtra(Constants.Actions.READ_BYTES, readBytes)
        sendBroadcast(intent)
    }
    ...
    private val gattClientCallback: BluetoothGattCallback = object : BluetoothGattCallback() {
        override fun onConnectionStateChange(gatt: BluetoothGatt, status: Int, newState: Int) {
            ...
            if (newState == BluetoothProfile.STATE_CONNECTED) {
                ...
                broadcastUpdate(Constants.Actions.GATT_CONNECTED, "Connected")
                Log.d(TAG, "Connected to the GATT server")
                gatt.discoverServices()
            } else if (newState == BluetoothProfile.STATE_DISCONNECTED) {
                broadcastUpdate(Constants.Actions.GATT_DISCONNECTED, "Disconnected")
            }
        }
        ...
        private fun readCharacteristic(characteristic: BluetoothGattCharacteristic) {
            ...
            broadcastUpdate(Constants.Actions.READ_CHARACTERISTIC ,msg)
            Log.d(TAG, "read: $msg")
        }
    ...
    private fun connectDevice(device: BluetoothDevice?) {
        // update the status
        broadcastUpdate(Constants.Actions.STATUS_MSG, "Connecting to ${device?.address}")
        bleGatt = device?.connectGatt(applicationContext, false, gattClientCallback)
    }
    ...
    fun disconnectGattServer(msg: String) {
        ...
        broadcastUpdate(Constants.Actions.GATT_DISCONNECTED, msg)
    }
